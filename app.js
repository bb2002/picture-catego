 ...
 */
 ...
 function readLinePromise(distFileLocation) {
 ...
 async function workStation() {
     ...
     for(distFile of fileListOfDist) {
         ...
         let isVideo = false             // 비디오의 경우, EXIF 데이터를 뽑을 수 없다.
 
         
         {   // 파일 형식에 따라서 video / photo 를 구분
             const extension = distFile.split(".")[1].toLowerCase()
             if(extension == "mp4" || extension == "avi" || extension == "mkv" || extension == "wmv" || extension == "mov" || extension == "flv" || extension == "3gp" || extension == "mts") {
         ...
         {   // EXIF 데이터를 기반으로 위치 파악
             if(!isVideo) {
                 ...
                 if(tags.DateTime && tags.DateTime.value && tags.DateTime.value[0]) {
                     ...
                     let a = dateStr.split(" ")[0].replace(/:/gi,"-").replace(/\//gi,"-")
                     ...
                     if(picDate.isValid()) {
                        ...
                        console.log(`[!] ${distFile}의 날짜 정보를 파싱 할 수 없습니다.`)
                        console.log(`[!] 예상 날짜는 ${a} 입니다. 인간이 파싱해서 알려주세요.`)
                        const picDate = await readLinePromise(distFileLocation)
                        moveToLocation += `/${picDate.format("YYYY. MM")}`
                     }
                 } else {
                 ...
                 console.log(`[!] ${distFile}은 비디오로 EXIF 데이터를 읽을 수 없습니다.`)
                 const picDate = await readLinePromise(distFileLocation)
                 moveToLocation += `/${picDate.format("YYYY. MM")}`
             }
         }
